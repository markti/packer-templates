build {
  ...
  provisioner "shell" {
    execute_command = local.execute_command
    inline = ["mkdir -p /home/mcserver/minecraft_bedrock/worlds"]
  }
  # Blobfuse v2 config drop
  provisioner "file" {
    source = "./files/blobfuse-config.yaml"
    destination = "/tmp/blobfuse-config.yaml"
  }
  provisioner "shell" {
    execute_command = local.execute_command
    inline = ["cp /tmp/blobfuse-config.yaml /home/mcserver/blobfuse-config.yaml"]
  }
